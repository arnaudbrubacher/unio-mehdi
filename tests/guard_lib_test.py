...
class TestEndToEndElection(TestCase):
    ...
    def step_0_configure_election(self) -> None:
        ...
        self.manifest = ElectionFactory().get_manifest_from_filename(os.path.join(root_path, "data", "FSSS", "fsss_manifest.json"))
        print(
            f"""
            {'-'*40}\n
            # Election Summary:
    ...
    def step_2_encrypt_votes(self) -> None:
        ...
        # Load some Ballots
        root_path = str(pathlib.Path(__file__).parent.parent.resolve())
        ballots_path = "/home/zero/o/unio/data/FSSS/ballots"
        for b in os.listdir(ballots_path):
            self.plaintext_ballots.append(BallotFactory()._get_ballot_from_file(os.path.join(ballots_path, b)))
